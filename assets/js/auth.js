...
async function login(email,password){
  ...
  const { data: profile, error: profileError } = await supabaseClient.from("profiles")
      .select("role, name, username").eq("id", session.user.id).single();
...
async function protectPage(){
  ...
  const { data: profile, error: profileError } = await supabaseClient.from("profiles")
      .select("id, role, username, name, email").eq("id", user.id).single();
  if(profileError || !profile){ await logout(); return null; }
...
async function addUser(name,email,password,role){
  ...
  if(!user){ alert("فشل في إنشاء المستخدم"); return false; }
